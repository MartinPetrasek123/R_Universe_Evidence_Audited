...
\begin{abstract}
We present a unified theoretical framework that begins with a non-geometric macroscopic scalar theory---Relational Capacity Dynamics (RCD)---and constructs a covariant preferred-foliation completion---Relational Foliation Gravity (RFG). The objective is not only to compare a modified expansion history with \(\Lambda\)CDM, but to test whether a phenomenological dark-sector model can be replaced by a deeper dynamical account with explicit variables, actions, constraints, local limits, observables, likelihoods, and falsification gates. The primitive variable of the scalar sector is a strictly positive relational-capacity field \(\C(\tau,\mathbf{x})\). We supply the complete ontology, dimensional structure, conservative action, full variational derivation, Hamiltonian formulation, Noether identities with explicit local currents, finite-response transport, emergent physical-time map, and linear stability analysis. The scalar sector is ghost-free for \(\rhoC>0\), gradient-stable for \(\kC>0\), and strictly hyperbolic with characteristic speed \(c_{\mathcal{C}}=\sqrt{\kC/\rhoC}\).
...
\text{observables}\;\longrightarrow\;
\text{likelihoods}\;\longrightarrow\;
\text{falsification gates}.
\]
...
Covariant KGB likelihood and evidence & Fixed point, conditional CMB--late-time record, exact posterior, converged production evidence, and native RSD audit executed & The separately defined KGB realization is evolved with photons, baryons, CDM, massless neutrinos, recombination, and lensing in native H--EFTCAMB. A fixed-input Planck--Pantheon+--DESI DR2 BAO--chronometer record, a converged exact posterior for that stated probe set, a local screening gate, and dedicated dynamic nested-sampling evidence calculations are executed. The matched local \(\Lambda\)CDM reference evidence is also converged. The resulting \(\Delta\log Z=+0.5241750\), or \(B_{{\rm KGB},\Lambda{\rm CDM}}=1.6891\), weakly favors KGB at the point-estimate level; the combined numerical evidence uncertainty \(0.5924116\) makes the comparison inconclusive at the achieved precision. RSD is kept as a numerical residual audit because its survey likelihood inputs are unavailable. \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\(\texttt{nlive}=250\), \(\texttt{dlogz}=0.3\), the
\(\texttt{multi}\) bound, and \(\texttt{rwalk}\) sampling. Finite call blocks
were treated only as checkpoint intervals, not as convergence: each sampler was
resumed with unchanged priors, physics, and accuracy until its live-point
estimate satisfied the stated stopping criterion. The final continuation blocks
used \(\texttt{maxcall}=50000\); the call counts below are cumulative over all
checkpointed blocks. The resulting marginal likelihoods are
\begin{table}[H]
\centering
\small
...
\midrule
Covariant KGB R-Universe & \(-1250.3771730\) & \(0.4241604\) & \(2500.7543461\) & \(157132\) & converged \\
Local \(\Lambda\)CDM reference & \(-1250.9013480\) & \(0.4135692\) & \(2501.8026961\) & \(151268\) & converged \\
\bottomrule
\end{tabular}
\end{table}
Thus
...
=+0.5241750\pm0.5924116,\\
-2\Delta\log Z&=-1.0483500,
\qquad
B_{{\rm KGB},\Lambda{\rm CDM}}=\exp(\Delta\log Z)=1.6891 .
\end{aligned}
\label{eq:kgb-lcdm-delta-logz}
\end{equation}
The uncertainty on \(\Delta\log Z\) in Eq.~\eqref{eq:kgb-lcdm-delta-logz} is the
quadrature combination of the two reported nested-sampling uncertainties,
treating the numerical errors of the independent runs as independent. With the
conventional evidence orientation, the point estimate weakly favors the present
covariant KGB realization over the local \(\Lambda\)CDM reference for the
declared prior volumes and probe set. The Bayes factor of \(1.6891\) is small,
and \(|\Delta\log Z|<\sigma_{\Delta\log Z}\); the calculation therefore does
not establish a decisive model preference at the achieved numerical precision.
It nevertheless closes the previous evidence gate with KGB on the favored side
of the completed comparison. The native RSD calculation remains outside this
evidence because the available compilation is not a survey-complete likelihood.
...
\item The production Bayesian evidence calculation in Sec.~\ref{sec:kgb-lcdm-evidence} is converged for the stated KGB and local \(\Lambda\)CDM models under their declared priors and the same CMB-plus-late-time probe family. It gives \(\log Z_{\rm KGB}=-1250.3771730\pm0.4241604\) and \(\log Z_{\Lambda{\rm CDM}}=-1250.9013480\pm0.4135692\), or \(\Delta\log Z=+0.5241750\pm0.5924116\) and \(B_{{\rm KGB},\Lambda{\rm CDM}}=1.6891\). Thus the point estimate weakly favors KGB, but the preference is not decisive at the achieved numerical precision. This evidence comparison is not an RFG-R or RFG-R\(\Xi\) likelihood evaluation and still excludes RSD as a survey likelihood.
\end{enumerate}
...
reported in Sec.~\ref{sec:kgb-lcdm-evidence} are taken from the final
converged local dynamic nested-sampling JSON/checkpoint/log records for the
KGB and \(\Lambda\)CDM runs. Those records report cumulative call counts
\(157132\) and \(151268\), respectively, and achieved stopping estimates
\(0.2989872\) and \(0.2991279\), both below the declared \(0.3\) threshold.
These evidence records and a machine-readable comparison summary are included
in the public
