...
When $p_k$ depends on estimated model parameters $\theta$--for example $p_0$, $K_\theta$, $P_M$, classifier calibration, or a covariance model--event uncertainty should be separated from parameter uncertainty:
\[
  \mathrm{Var}(N_{\VST}\mid D)
  =
  \E_\theta\!\left[\mathrm{Var}(N_{\VST}\mid D,\theta)\mid D\right]
  +
  \mathrm{Var}_\theta\!\left(\E[N_{\VST}\mid D,\theta]\mid D\right).
\]
...
\]
is the set of threshold-positive samples or primitive activations, while
\[
...
False successes among committed & 4,374,339 & 535,442 \\
...
False-success fraction, committed denominator & 0.840 & 0.407 \\
False-success fraction, resolved denominator & 0.945 & 0.456 \\
False-acceptance rate against latent truth & 0.023 & 0.159 \\
False-rejection rate against latent truth & 0.625 & 0.157 \\
\bottomrule
\end{tabular}
\end{table}
...
System A mean \VSTE{} 95\% CI & [0.00221, 0.00223] \\
System B mean \VSTE{} 95\% CI & [0.00687, 0.00690] \\
...
The raw-activity ranking and VSTF ranking disagree in 78\% of the independent batches under the locked operating point. System A is slightly higher on raw activity per supplied energy, while System B is approximately 3.10 times higher on valid VST per supplied energy. The sensitivity sweep is deliberately reported rather than hidden: ranking reversal is not universal under all noise levels, and the VSTF claim is strongest in regimes where noise, retention failure, and cost accounting create a measurable separation between activity and retained domain-valid outcome. This is precisely the incremental claim VSTF must earn; it must be shown, with uncertainty, that the acceptance layer changes rankings that would be drawn from raw activity, crossings, or committed first-passage events alone.
