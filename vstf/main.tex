...
\[
  ...
  \1\{V_{\mathrm{dom}}=1\}.
\]
Here $A_k=g(\mathcal{Y}_k;\mathcal{P})$ is the decision made by the locked protocol $\mathcal{P}$ from the observed event packet $\mathcal{Y}_k$. Only events with $A_k=1$ enter the operational VST count
\[
...
Because VSTF often involves measurement, readout, feedback, and post-selection on accepted events, any use of work/free-energy equalities must declare whether the controller is inside the boundary and whether conditioning on acceptance changes the relevant work distribution. Nonequilibrium work relations such as Jarzynski's equality and feedback-control generalizations constrain ensemble statistics of work and show that measurement and information terms can alter the relevant work relations \cite{jarzynski1997,sagawa2010}. They should not be interpreted as deterministic lower bounds on every accepted stochastic trajectory without an additional derivation. These thermodynamic relations are conditional specializations of VSTF, not the general foundation of the framework.

\begin{quote}
\textbf{Thermodynamic convention.} Any equality connecting work, free-energy change, and entropy production is implementation-specific. A study must specify the thermodynamic state function, whether $\Delta F$ is equilibrium or nonequilibrium free energy, bath temperature, entropy units, feedback/controller boundary, post-selection rule, and initial/final ensemble assumptions before interpreting a VSTF denominator as a thermodynamic expression. VSTF itself does not imply a universal thermodynamic upper bound on \VSTE{}; such a bound requires a separate sample-level resource derivation.
\end{quote}
...
\begin{figure}[h]
\centering
\IfFileExists{vstf_hmm_benchmark.pdf}{%
  \includegraphics[width=0.96\linewidth]{vstf_hmm_benchmark.pdf}%
}{%
  \fbox{\parbox{0.92\linewidth}{\centering Missing generated figure file: \texttt{vstf\_hmm\_benchmark.pdf}. Regenerate with \texttt{python3 vstf\_hmm\_benchmark.py}.}}%
}
...
\begin{table}[h]
\centering
\caption{Aggregate summary over 100 independent batches, each with 10,000 trajectories per system.}
\label{tab:hmm-summary}
\begin{tabular}{lrr}
\toprule
Quantity & System A & System B \\
\midrule
Raw activity samples & 20,628,730 & 16,694,142 \\
Crossing events & 7,793,970 & 1,555,412 \\
Committed events & 5,210,564 & 1,314,145 \\
Retained events & 351,197 & 725,299 \\
Valid VST events & 252,478 & 638,268 \\
Resolved committed events failing downstream VST criteria & 4,374,339 & 535,442 \\
Latent-ground-truth valid events & 415,864 & 660,793 \\
True positives $(A_k=1,Z_k=1)$ & 155,923 & 556,952 \\
False acceptances & 96,555 & 81,316 \\
False rejections & 259,941 & 103,841 \\
Pending/right-censored events & 583,747 & 140,435 \\
Full-boundary energy & 113,755,292 & 92,723,514 \\
Raw activity per energy & 0.18134 & 0.18004 \\
Crossing events per energy & 0.06852 & 0.01677 \\
Committed events per energy & 0.04580 & 0.01417 \\
Retained events per energy & 0.00309 & 0.00782 \\
...
Positive predictive value $TP/(TP+FP)$ & 0.618 & 0.873 \\
Sensitivity $TP/(TP+FN)$ & 0.375 & 0.843 \\
False discovery fraction $FP/(TP+FP)$ & 0.382 & 0.127 \\
\bottomrule
\end{tabular}
\end{table}
...
Quantity & Value \\
\midrule
Independent batches & 100 \\
Trajectories per batch per system & 10,000 \\
$\Pr(R_A>R_B\ \mathrm{and}\ \VSTE_B>\VSTE_A)$ & 0.78 \\
...
Mean raw-efficiency ratio $R_A/R_B$ & 1.007 \\
System A mean \VSTE{} Monte Carlo 95\% interval & [0.00221, 0.00223] \\
System B mean \VSTE{} Monte Carlo 95\% interval & [0.00687, 0.00690] \\
Sensitivity reversal probability, $\sigma$ factor 0.75 & 0.00 across $\tau_\ast\in\{4,6,10\}$ \\
Sensitivity reversal probability, $\sigma$ factor 1.00 & 0.60--0.80 across $\tau_\ast\in\{4,6,10\}$ \\
...
\]
For decision-state use, the VST event is the accepted alarm or decision transition, not the patient's biological transition itself. Until patient-level calibration, raw-vital baselines, alarm burden, decision curves, and external validation are completed, PSI-like outputs should be treated as component-level instability features rather than clinical VST events. A minimal clinical validation panel should include calibration-in-the-large, calibration slope or curve, discrimination, Brier score, net benefit, alarm burden, and an explicit baseline such as NEWS2 or a raw-vital model \cite{news2,trwscore,physionet,harutyunyan2019}. Splits must be by patient, not prediction instance, and temporal generalization should be tested by training on earlier cohorts and evaluating on later cohorts when the data permit. Reporting should follow TRIPOD+AI for prediction-model manuscripts; methodological quality, risk of bias, and applicability should be assessed with PROBAST+AI \cite{tripod,collins2024tripodai,moons2025probastai}. TRIPOD+AI is a reporting standard, not proof of model validity; PROBAST+AI is an appraisal framework, not a clinical-utility test. Clinical utility still requires a separate evaluation of patient-relevant benefit, harm, and decision consequences.
...
\begin{enumerate}
  ...
  \item Split development, calibration, and external validation cohorts by patient; additionally split by hospital, unit, or time period where possible.
...
\begin{enumerate}
  ...
  \item Include an independent functional endpoint when the reset is claimed to have biological utility.
...
\begin{sloppypar}
\textbf{Data and code availability.} This manuscript is a theoretical and methodological synthesis with a synthetic HMM demonstration. The simulation script, aggregate summary, Monte Carlo summary, sensitivity sweep, ablation summary, random-parameter stress test, event-level audit sample, reproducibility manifest, requirements file, and figure are provided as \path{vstf_hmm_benchmark.py}, \path{vstf_hmm_benchmark_summary.csv}, \path{vstf_hmm_benchmark_monte_carlo.csv}, \path{vstf_hmm_benchmark_sensitivity.csv}, \path{vstf_hmm_benchmark_ablations.csv}, \path{vstf_hmm_random_parameter_stress.csv}, \path{vstf_hmm_event_sample.csv}, \path{REPRODUCIBILITY.md}, \path{requirements.txt}, and \path{vstf_hmm_benchmark.pdf}. Domain-specific empirical claims require separate reproducibility packages, locked datasets, code, and analysis logs.
\end{sloppypar}
...
\bibitem{bartocci2013rv}
E. Bartocci, B. Bonakdarpour, and Y. Falcone.
Runtime verification of embedded real-time systems.
\emph{Formal Methods in System Design}, 43:203--239, 2013.
