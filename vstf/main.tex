...
\begin{definition}[Operational decision and latent event truth]
...
When a latent or adjudicated ground truth exists, define separately
\[
  Z_k\in\{0,1\},
\]
where $Z_k=1$ means that event $k$ is truly valid under the target state, retention, and domain-validity semantics, independently of whether the measurement rule accepted it. In noiseless task benchmarks $A_k$ and $Z_k$ may coincide, but under measurement uncertainty they are distinct. False acceptance and false rejection are defined as
\[
  FP=\#\{k:A_k=1,\ Z_k=0\},
  \qquad
  FN=\#\{k:A_k=0,\ Z_k=1\}.
...
\begin{definition}[Probabilistic latent validity]
When state-validity predicates cannot be observed without error, the hard operational decision $A_k$ should be accompanied by a posterior probability of latent validity,
\[
...
\begin{quote}
\textbf{Separation principle.} The latent truth of an event, the measurement record, the operational decision, and the scientific claim are distinct objects:
\[
  Z_k\ \neq\ \mathcal{Y}_k\ \neq\ A_k\ \neq\ \mathrm{claim}.
\]
...
\begin{quote}
\textbf{Thermodynamic convention.} Any equality connecting work, free-energy change, and entropy production is implementation-specific. A study must specify the thermodynamic state function, whether $\Delta F$ is equilibrium or nonequilibrium free energy, bath temperature, entropy units, feedback/controller boundary, and initial/final ensemble assumptions before interpreting a VSTF denominator as a thermodynamic lower-bound expression.
\end{quote}
...
\begin{itemize}
  ...
  \item \href{https://github.com/MartinPetrasek123/R_Universe_Evidence_Audited/blob/main/vstf/vstf_hmm_benchmark_monte_carlo.csv}{\texttt{vstf\_hmm\_benchmark\_monte\_carlo.csv}}: multi-seed robustness summary used in Table 4.
  ...
  \item \href{https://github.com/MartinPetrasek123/R_Universe_Evidence_Audited/blob/main/vstf/vstf_hmm_benchmark_ablations.csv}{\texttt{vstf\_hmm\_benchmark\_ablations.csv}}: controlled A/B ablation summary.
...
\]
Here $\mu_{\mathrm{path}}$, $\mu_{\mathrm{ref}}$, feature scaling, module definitions, and covariance estimate $\Sigma$ must be frozen from an independent training/reference cohort before confirmatory testing. The protocol must state whether $\Sigma$ is a pooled covariance, a reference-state covariance, a pathological-state covariance, or two separate covariance matrices for the two distances. If the number of features is not small relative to the training cohort, $\Sigma^{-1}$ must be estimated with a pre-specified shrinkage, diagonal, low-rank, or regularized covariance protocol, for example
\[
...
\]
These inequalities establish conditional dependence, not by themselves causal voluntary control. A confirmatory design should use randomized cue epochs, sham or no-intent epochs, artifact masking, pre-specified lag windows, negative temporal controls, and clustered inference by participant and session. A compact model is
\[
  \mathrm{logit}\,\Pr(Y=1)
  =
