...
\begin{enumerate}
  ...
  \]
  Distinguishability can be treated as a protocol-level validation flag $Q_{\mathrm{dist}}(M,\{C_i\})=1$ for the measurement system, while localization remains event-specific.
...
\begin{definition}[Operational decision and latent/reference event validity]
For candidate event $e_k$, define the operational VSTF decision
\[
  A_k =
  Q_{\mathrm{dist}}(\mathcal{P},M,\{C_i\})
  L_k
  R_k(\tau_\ast)
  V_{\mathrm{dom},k}.
\]
Here $Q_{\mathrm{dist}}(\mathcal{P},M,\{C_i\})\in\{0,1\}$ is a protocol-level measurement-system qualification flag, $L_k$ is event-level localization and segmentation success, $R_k(\tau_\ast)$ is event-level retention, and $V_{\mathrm{dom},k}$ is event-level domain admissibility. This separates a reusable confusion-matrix or distinguishability validation from predicates that are evaluated on each event. $A_k=g(\mathcal{Y}_k;\mathcal{P})$ is the decision made by the locked protocol $\mathcal{P}$ from the observed event packet $\mathcal{Y}_k$. Only events with $A_k=1$ enter the operational VST count
\[
  N_{\VST}^{\mathrm{op}}=\sum_k A_k.
\]

When a latent or independently adjudicated reference-standard validity exists, define separately
\[
  Z_k\in\{0,1\},
\]
where $Z_k=1$ means that event $k$ is valid under the target state, retention, and domain-validity semantics according to that reference standard, independently of whether the measurement rule accepted it. In hardware benchmarks this may be a latent simulator or reference state; in clinical or biological settings it is often a fallible but independently justified reference standard. In noiseless task benchmarks $A_k$ and $Z_k$ may coincide, but under measurement uncertainty they are distinct. False acceptance and false rejection are defined as
\[
  FP=\#\{k:A_k=1,\ Z_k=0\},
  \qquad
  FN=\#\{k:A_k=0,\ Z_k=1\}.
...
\begin{definition}[Probabilistic latent/reference validity]
...
\[
  p_k=\Pr(Z_k=1\mid \mathcal{Y}_k,D_{\mathrm{train}}),
  \qquad
  \E[N_{\VST}^{\mathrm{ref}}\mid Y,D_{\mathrm{train}}]=\sum_k p_k.
\]
Here $\mathcal{Y}_k$ is the event-specific data packet: the observations used for commitment, validation, retention, distinguishability, domain validity, and censoring status. If $A_k$ is a deterministic function of $\mathcal{Y}_k$ under a locked protocol, then $\Pr(A_k=1\mid\mathcal{Y}_k)=A_k$ is trivial; the nontrivial uncertainty concerns $Z_k$, not the existence of the operational decision. The variance of the probabilistic reference-valid count should be reported as
\[
  \mathrm{Var}\!\left(\sum_k Z_k\mid \mathcal{Y},D_{\mathrm{train}}\right)
  =
  \sum_k p_k(1-p_k)
  +2\sum_{k<\ell}\mathrm{Cov}(Z_k,Z_\ell\mid \mathcal{Y},D_{\mathrm{train}}),
\]
...
\[
  Z_k\ \neq\ \mathcal{Y}_k\ \neq\ A_k\ \neq\ \mathrm{claim}.
\]
The measurement record $\mathcal{Y}_k$ supports an operational adjudication $A_k$ under a locked protocol, and may support posterior statements about $Z_k$ under an explicit validation model. A scientific claim is then restricted by the declared evidence level, baseline comparison, uncertainty analysis, domain predicate, and denominator completeness.
\end{quote}
...
When $p_k=\Pr(Z_k=1\mid\mathcal{Y}_k,D_{\mathrm{train}})$ depends on estimated model parameters $\theta$--for example $p_0$, $K_\theta$, $P_M$, classifier calibration, or a covariance model--event uncertainty should be separated from parameter uncertainty:
\[
  \mathrm{Var}(N_{\VST}^{\mathrm{ref}}\mid D)
  =
  \E_\theta\!\left[\mathrm{Var}(N_{\VST}^{\mathrm{ref}}\mid D,\theta)\mid D\right]
  +
  \mathrm{Var}_\theta\!\left(\E[N_{\VST}^{\mathrm{ref}}\mid D,\theta]\mid D\right).
\]
The first term is event uncertainty; the second is model or parameter uncertainty. Posterior sampling, nested bootstrap, clustered bootstrap, or another pre-specified procedure should be used when model-based probabilities are reported.
\end{definition}
...
Energy per valid VST & 3.20 & 2.10 \\
...
Full-boundary energy & 34,127,354 & 27,815,588 \\
Raw activity per energy & 0.18140 & 0.17972 \\
Crossing events per energy & 0.06852 & 0.01677 \\
Committed events per energy & 0.04581 & 0.01415 \\
Retained events per energy & 0.00308 & 0.00782 \\
Valid VST per energy & 0.00222 & 0.00688 \\
...
$\Pr(R_A>R_B\ \mathrm{and}\ \VSTE_B>\VSTE_A)$ & 0.77 \\
...
Mean $\VSTE_B/\VSTE_A$ & 3.10 \\
Mean raw-efficiency ratio $R_A/R_B$ & 1.009 \\
System A mean \VSTE{} Monte Carlo 95\% interval & [0.00220, 0.00223] \\
System B mean \VSTE{} Monte Carlo 95\% interval & [0.00686, 0.00691] \\
...
\midrule
Dynamics only & 1.00 & 1.00 & 0.00 & 2.36 \\
Noise only & 0.00 & 1.00 & 0.00 & 1.75 \\
Domain only & 0.50 & 0.63 & 0.50 & 1.22 \\
Cost only & 0.00 & 0.00 & 0.00 & 1.10 \\
Domain+cost only & 0.00 & 0.00 & 0.00 & 1.34 \\
Retention-challenge & 0.00 & 0.00 & 1.00 & 1.32 \\
Combined with equal domain probability & 0.75 & 1.00 & 0.00 & 2.53 \\
Combined & 0.75 & 1.00 & 0.00 & 3.08 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

The raw-sample ranking and VSTF ranking disagree in 77\% of the independent batches under the locked operating point. More importantly, the event-level crossing and committed-event baselines also rank System A above System B in every batch, while \VSTE{} ranks System B above System A. The retained-event baseline already ranks System B above System A in the main locked operating point, which is expected because retention is one of the core VSTF filters; this makes clear that the benchmark does not claim VSTF defeats every simpler event-level metric. To test incremental value beyond retention alone, the retention-challenge ablation locks the comparison to a regime in which the retained-event baseline ranks System A above System B in all eight ablation batches while \VSTE{} ranks System B above System A. This ablation isolates the residual contribution of domain-validity and cost governance after event retention has already been counted. The sensitivity and ablation results are deliberately reported rather than hidden: ranking reversal is not universal under all noise levels or all parameter changes, and the VSTF claim is strongest in regimes where noise, retention failure, domain filtering, and cost accounting create a measurable separation between activity and retained domain-valid outcome. The accompanying random-parameter stress file samples eight additional A/B pairs over transition, noise, domain-validity, and energy-ledger ranges; it is a broader robustness check, not a substitute for a full domain-specific empirical benchmark. The reported intervals are Monte Carlo intervals across synthetic batches, not confidence intervals for a physical population parameter.
...
\midrule
HMM, independent domain & 0.67 & 1.00 & 0.00 & 3.11 & 0.871 \\
HMM, correlated domain & 0.67 & 1.00 & 0.00 & 2.68 & 0.954 \\
HSMM, independent domain & 1.00 & 1.00 & 0.00 & 5.83 & 0.876 \\
HSMM, correlated domain & 1.00 & 1.00 & 0.00 & 5.08 & 0.957 \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}
...
When validity is observed under uncertainty, the operational decision $A_k$ and reference/latent validity $Z_k$ are distinct; reporting should show both operational counts and any available latent or adjudicated reference-standard validation.
...
\begin{sloppypar}
\textbf{Data and code availability.} This manuscript is a theoretical and methodological synthesis with a synthetic HMM demonstration. The simulation script, aggregate summary, Monte Carlo summary, sensitivity sweep, ablation summary, random-parameter stress test, scenario stress test, event-level audit sample, reproducibility manifest, requirements file, workflow figure, and benchmark figure are provided as \path{vstf_hmm_benchmark.py}, \path{vstf_hmm_benchmark_summary.csv}, \path{vstf_hmm_benchmark_monte_carlo.csv}, \path{vstf_hmm_benchmark_sensitivity.csv}, \path{vstf_hmm_benchmark_ablations.csv}, \path{vstf_hmm_random_parameter_stress.csv}, \path{vstf_hmm_scenario_stress.csv}, \path{vstf_hmm_event_sample.csv}, \path{REPRODUCIBILITY.md}, \path{requirements.txt}, \path{vstf_event_cascade.pdf}, and \path{vstf_hmm_benchmark.pdf}. Domain-specific empirical claims require separate reproducibility packages, locked datasets, code, and analysis logs.
\end{sloppypar}
