...
\newcommand{\VSTFgraphic}[3]{%
  \IfFileExists{#1}{%
    ...
    \fbox{%
      ...
      The source artifact is available at:\\
      \href{#3}{\texttt{#1}}
      \vspace{1em}
      \end{minipage}%
    }%
...
\]
subject to a maximum admissible cost and one-to-one assignment. Unmatched accepted candidate events contribute to false acceptance; unmatched reference-valid events contribute to false rejection.
\end{definition}

\begin{definition}[Operational decision and matched reference validity]
For detected candidate event $\hat e_k$, define the operational VSTF decision
\[
  A_k =
...
\[
  p_k=\Pr(Z^{\mathrm{match}}_k=1\mid \mathcal{Y}_k,D_{\mathrm{train}}),
  \qquad
  \E[N_{\mathrm{ref,cand}}\mid Y,D_{\mathrm{train}}]=\sum_{\hat e_k\in\widehat{\mathcal{E}}_{\mathcal{P}}} p_k.
\]
...
When $p_k=\Pr(Z^{\mathrm{match}}_k=1\mid\mathcal{Y}_k,D_{\mathrm{train}})$ depends on estimated model parameters $\theta$--for example $p_0$, $K_\theta$, $P_M$, classifier calibration, matching tolerance, or a covariance model--event uncertainty should be separated from parameter uncertainty:
\[
...
\]
The first term is event uncertainty; the second is model or parameter uncertainty. Posterior sampling, nested bootstrap, clustered bootstrap, or another pre-specified procedure should be used when model-based probabilities are reported. The probability model used for $p_k$ should be calibrated out of sample or validated outside the confirmatory set; otherwise $\sum_k p_k$ is only a model-implied count under the fitted assumptions.
\end{definition}
...
\]
where $\bar{L}_{\mathcal{A}}=N_{\VST}^{-1}\sum_{k\in\mathcal{A}}L_k$. Only in a specific physical implementation where a separate derivation justifies substituting $L_k=\Delta F_k>0$ as an eventwise resource bound does this become a free-energy bound. If a uniform samplewise bound $L_k\geq L_{\min}>0$ is justified for all accepted events, then $\widehat{\VSTE}_{[a,b]}\leq 1/L_{\min}$. No useful positive bound of this type follows from events with $L_k\leq0$ or from ensemble-average thermodynamic relations alone.
...
\begin{quote}
\textbf{Thermodynamic specialization.} VSTF itself imposes no universal relation between accepted-event counts and work, heat, entropy production, or free-energy change. For a particular physical implementation, an independently derived ensemble thermodynamic relation may be imposed on the event-generating subsystem. Likewise, a sample-level upper bound on \VSTE{} exists only if every accepted event admits an independently justified positive resource lower bound $L_k$, additive over non-overlapping resource segments. In that case $E_{\mathrm{sup}}^{\mathcal{B}}[a,b]\geq\sum_{k\in\mathcal{A}}L_k$ and $\widehat{\VSTE}_{[a,b]}\leq1/\bar{L}_{\mathcal{A}}$. Substitution of $L_k$ by a free-energy quantity requires a separate physical derivation; it is not implied by VSTF.
\end{quote}
...
\]
counts threshold-positive samples or primitive activations. This is an occupancy-like activity measure, not an event set. The event universe is instead
\[
...
If every accepted event $k\in\mathcal{A}$ admits an independently justified positive sample-level resource lower bound $L_k>0$, and resource contributions are additive over non-overlapping segments inside $\mathcal{B}$, then
\[
  E_{\mathrm{sup}}^{\mathcal{B}}\geq\sum_{k\in\mathcal{A}}L_k,
  \qquad
  \widehat{\VSTE}\leq
  \frac{N_{\VST}}{\sum_{k\in\mathcal{A}}L_k}
  =
  \frac{1}{\bar L_{\mathcal{A}}}.
\]
\end{lemma}
...
\begin{figure}[h]
\centering
\VSTFgraphic{vstf_event_cascade.pdf}{0.98}{https://github.com/MartinPetrasek123/R_Universe_Evidence_Audited/blob/main/vstf/vstf_event_cascade.pdf}
...
\begin{figure}[h]
\centering
\VSTFgraphic{vstf_hmm_benchmark.pdf}{0.96}{https://github.com/MartinPetrasek123/R_Universe_Evidence_Audited/blob/main/vstf/vstf_hmm_benchmark.pdf}
\caption{Hidden-Markov benchmark showing nested attrition from raw activity to valid VST events, latent-ground-truth error rates, multi-seed ranking robustness, and a small sensitivity sweep. System A produces more raw activity per supplied energy, but most committed events fail validation or retention. System B produces fewer raw events but substantially higher valid VST yield per supplied energy.}
...
\]
represent voluntary intent, stimulation, EMG, movement, sensory feedback, autonomic state, and pain/spasticity/fatigue. The latent intent variable $U(t)$ is not directly observed; it must be connected to an independent measurement channel $U^{\mathrm{meas}}(t)$ through a declared measurement model or adjudication rule before it can support a VST claim.
...
\begin{sloppypar}
\textbf{Data and code availability.} This manuscript is a theoretical and methodological synthesis with a synthetic HMM demonstration. The simulation script, aggregate summary, Monte Carlo summary, sensitivity sweep, sampling-resolution stress test, ablation summary, random-parameter stress test, scenario stress test, event-level audit sample, reproducibility manifest, requirements file, workflow figure, and benchmark figure are provided as \path{vstf_hmm_benchmark.py}, \path{vstf_hmm_benchmark_summary.csv}, \path{vstf_hmm_benchmark_monte_carlo.csv}, \path{vstf_hmm_benchmark_sensitivity.csv}, \path{vstf_hmm_sampling_resolution.csv}, \path{vstf_hmm_benchmark_ablations.csv}, \path{vstf_hmm_random_parameter_stress.csv}, \path{vstf_hmm_scenario_stress.csv}, \path{vstf_hmm_event_sample.csv}, \path{REPRODUCIBILITY.md}, \path{requirements.txt}, \path{vstf_event_cascade.pdf}, and \path{vstf_hmm_benchmark.pdf}. Domain-specific empirical claims require separate reproducibility packages, locked datasets, code, and analysis logs.
\end{sloppypar}
